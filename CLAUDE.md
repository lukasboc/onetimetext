...
OneTimeText is a Laravel 8 web app (PHP 7.3 / 8.0+) for sharing self-destructing secrets: a user pastes text, gets a URL with a random key, and the recipient can read it exactly once before the row is deleted. The UI is primarily German. A paid "Pro" tier is layered on top via Laravel Cashier + Stripe.
